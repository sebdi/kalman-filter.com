...
The measurement residual \\( \nu (k) \\) is the difference of the measurement prediction \\( \mathbf{\hat{z}}(k) \\) and the true value for the measurement \\( \mathbf{z}(k) \\).
The latter is just the true state vector projected to the measurement space with 
...
This is particularly relevant for practice, since in practice often only ground truth data is available for the measurement space.
